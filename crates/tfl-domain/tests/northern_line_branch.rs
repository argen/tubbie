...
/// Helper: assert a specific inferred Direction.
fn infer(platform: &str, direction: &str, towards: &str) -> Direction {
...
fn northbound_via_cx_edgware() {
    ...
    let d = infer("Northbound - Platform 1", "outbound", "Edgware via CX");
    assert_eq!(
        d,
        Direction::Northbound {
            via: Some(NorthernBranch::CharingCross)
        }
    );
}
...
fn southbound_via_cx_battersea() {
    // Observed in fixtures/arrivals/940GZZLUBZP.json (Belsize Park)
    let d = infer("Southbound - Platform 2", "inbound", "Battersea via CX");
    assert_eq!(
        d,
...
fn southbound_via_bank_morden() {
    // Observed in fixtures/arrivals/940GZZLUKSX.json (King's Cross)
    let d = infer("Southbound - Platform 8", "inbound", "Morden via Bank");
    assert_eq!(
        d,
        Direction::Southbound {
            via: Some(NorthernBranch::Bank)
...
fn northbound_high_barnet_no_branch_in_towards() {
    ...
    let d = infer("Northbound - Platform 4", "outbound", "High Barnet");
    assert_eq!(d, Direction::Northbound { via: None });
}

#[test]
fn southbound_no_towards() {
    // Empty `towards` → branch cannot be determined.
    let d = infer("Southbound - Platform 3", "inbound", "");
    assert_eq!(d, Direction::Southbound { via: None });
}
...
fn platform_prefix_overrides_direction_field() {
    ...
    let d = infer("Northbound - Platform 4", "inbound", "High Barnet via Bank");
    assert_eq!(
        d,
        Direction::Northbound {
            via: Some(NorthernBranch::Bank)
